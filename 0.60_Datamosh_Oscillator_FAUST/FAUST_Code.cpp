...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void instanceConstants(int sample_rate) {
		fSampleRate = sample_rate;
		fConst0 = (0.009765625f / std::min<float>(192000.0f, std::max<float>(1.0f, float(fSampleRate))));
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addNumEntry("freq", &fEntry1, FAUSTFLOAT(1.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(1024.0f), FAUSTFLOAT(1.0f));
		ui_interface->addNumEntry("gain", &fEntry0, FAUSTFLOAT(1.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(1024.0f), FAUSTFLOAT(1.0f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow1 = (4.65661287e-10f * ((0.0048828125f * fSlow0) + 1.0f));
		float fSlow2 = float(fEntry1);
		int iSlow3 = int(fSlow2);
		float fSlow4 = (fConst0 * (fSlow2 * fSlow0));
		for (int i0 = 0; (i0 < count); i0 = (i0 + 1)) {
			iVec0[(IOTA & 2047)] = ((1103515245 * iRec1[1]) + 12426538);
			iRec1[0] = iVec0[((IOTA - iSlow3) & 2047)];
			fRec0[0] = (fSlow1 * (float(iRec1[0]) * (fSlow4 + (fRec0[1] - float(int((fSlow4 + fRec0[1])))))));
			output0[i0] = FAUSTFLOAT(fRec0[0]);
			iVec1[(IOTA & 2047)] = ((1103515245 * iRec3[1]) + 48423276);
			iRec3[0] = iVec1[((IOTA - iSlow3) & 2047)];
			fRec2[0] = (fSlow1 * (float(iRec3[0]) * (fSlow4 + (fRec2[1] - float(int((fSlow4 + fRec2[1])))))));
	...
	FAUST_ADDNUMENTRY("freq", fEntry1, 1.0f, 1.0f, 1024.0f, 1.0f);
	FAUST_ADDNUMENTRY("gain", fEntry0, 1.0f, 1.0f, 1024.0f, 1.0f);

	#define FAUST_LIST_ACTIVES(p) \
		p(NUMENTRY, freq, "freq", fEntry1, 1.0f, 1.0f, 1024.0f, 1.0f) \
		p(NUMENTRY, gain, "gain", fEntry0, 1.0f, 1.0f, 1024.0f, 1.0f) \
