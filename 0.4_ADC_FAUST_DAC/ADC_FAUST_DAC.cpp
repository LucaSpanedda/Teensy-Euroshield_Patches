...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "ADC_FAUST_DAC.dsp");
		m->declare("name", "ADC_FAUST_DAC");
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("ADC_FAUST_DAC");
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			output0[i] = FAUSTFLOAT(fVec0[((IOTA - 1000) & 1023)]);
	...
	#define FAUST_FILE_NAME "ADC_FAUST_DAC.dsp"
...
ADC_FAUST_DAC::ADC_FAUST_DAC() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void ADC_FAUST_DAC::updateImp(void)
{
#if MIDICTRL
...
void ADC_FAUST_DAC::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void ADC_FAUST_DAC::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float ADC_FAUST_DAC::getParamValue(const std::string& path)
{
