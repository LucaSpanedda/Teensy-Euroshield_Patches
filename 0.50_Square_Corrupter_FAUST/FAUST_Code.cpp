...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void instanceConstants(int sample_rate) {
		fSampleRate = sample_rate;
		fConst0 = (0.100000001f / std::min<float>(192000.0f, std::max<float>(1.0f, float(fSampleRate))));
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("FAUST_Code");
		ui_interface->addNumEntry("freq", &fEntry0, FAUSTFLOAT(1.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(1024.0f), FAUSTFLOAT(1.0f));
		ui_interface->addNumEntry("gain", &fEntry1, FAUSTFLOAT(1.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(1024.0f), FAUSTFLOAT(1.0f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow1 = (638.219482f * fSlow0);
		float fSlow2 = (fConst0 * (fSlow0 * (fSlow1 - float(int(fSlow1)))));
		int iSlow3 = int(float(fEntry1));
		float fSlow4 = (1137.69568f * fSlow0);
		float fSlow5 = (fConst0 * (fSlow0 * (fSlow4 - float(int(fSlow4)))));
		for (int i0 = 0; (i0 < count); i0 = (i0 + 1)) {
			fRec0[0] = (fSlow2 + (fRec0[1] - float(int((fSlow2 + fRec0[1])))));
			float fTemp0 = (fRec0[0] + -0.5f);
			int iTemp1 = (fTemp0 < 0.0f);
			iVec0[(IOTA & 2047)] = (0 - iTemp1);
			int iTemp2 = (fTemp0 > 0.0f);
			iVec1[(IOTA & 2047)] = iTemp2;
			output0[i0] = FAUSTFLOAT(((iVec0[((IOTA - iSlow3) & 2047)] + (iVec1[((IOTA - iSlow3) & 2047)] + iTemp1)) - iTemp2));
			fRec1[0] = (fSlow5 + (fRec1[1] - float(int((fSlow5 + fRec1[1])))));
			float fTemp3 = (fRec1[0] + -0.5f);
			int iTemp4 = (fTemp3 < 0.0f);
			iVec2[(IOTA & 2047)] = (0 - iTemp4);
			int iTemp5 = (fTemp3 > 0.0f);
			iVec3[(IOTA & 2047)] = iTemp5;
			output1[i0] = FAUSTFLOAT(((iVec2[((IOTA - iSlow3) & 2047)] + (iVec3[((IOTA - iSlow3) & 2047)] + iTemp4)) - iTemp5));
			fRec0[1] = fRec0[0];
			IOTA = (IOTA + 1);
			fRec1[1] = fRec1[0];
		}
	...
	#define FAUST_FILE_NAME "FAUST_Code.dsp"
	...
	FAUST_ADDNUMENTRY("freq", fEntry0, 1.0f, 1.0f, 1024.0f, 1.0f);
	FAUST_ADDNUMENTRY("gain", fEntry1, 1.0f, 1.0f, 1024.0f, 1.0f);

	#define FAUST_LIST_ACTIVES(p) \
		p(NUMENTRY, freq, "freq", fEntry0, 1.0f, 1.0f, 1024.0f, 1.0f) \
		p(NUMENTRY, gain, "gain", fEntry1, 1.0f, 1.0f, 1024.0f, 1.0f) \
...
FAUST_Code::FAUST_Code() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void FAUST_Code::updateImp(void)
{
#if MIDICTRL
...
void FAUST_Code::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void FAUST_Code::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float FAUST_Code::getParamValue(const std::string& path)
{
